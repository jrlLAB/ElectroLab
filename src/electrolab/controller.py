...
dx0 = 4450  # dx_initial from the home position
dy0 = 2850  # dy_initial from the home position
dx = 2000  # dx for the nearest right cell
dy = 4000  # dy for the nearest upper cell
head_12 = np.array([1920, 400])
head_23 = np.array([-1650, -1140])
head_13 = np.array([-600, -680])
nozzle12 = np.array([-280, 400])

# Nitrogen (or Argon) nozzle global variables:
nozzle_n2_n = 1  # nitrogen-blowing nozzle number (1: drying, 2: bubbling)
nozzle12_n2 = np.array([870, 0])
head_23_n2 = np.array([-1680, -1140])
...
class Move_head(Move):
    ...
    def __init__(self, cellA, cellB, wait_time=1):
        ...
        route = np.array([int(xB) - int(xA), int(yB) - int(yA)]) * np.array([dx, dy])
        if xA == "0":
            route[0] = route[0] + dx0 - dx
            route[1] = route[1] + dy0 - dy
        elif xB == "0":
            route[0] = route[0] - dx0 + dx
            route[1] = route[1] - dy0 + dy
...
class Dispense(Motor):
    ...
    def vol_correction(self, volume, p=[0, 0, 0, 0]):
        ...
        self.wait_time[0] = 0
        self.wait_time[2] = abs(
            slope * vol * 11 * (self.speed / 1000) ** 2 / (9.1 * self.speed)
        )  # previously 1000
    ...
    def run(self):
        ...
        if nozzle_n == 1:
            initialization = "<PUMP2, " + speed + ", " + str(self.motor_values[0]) + ">"
            remove_drip = "<PUMP2, " + speed + ", " + str(self.motor_values[1]) + ">"
            dispense = "<PUMP2, " + speed + ", " + str(self.motor_values[2]) + ">"
            remove_drip2 = "<PUMP2, " + speed + ", " + str(self.motor_values[3]) + ">"
            idle = "<PUMP2, " + speed + ", " + str(self.motor_values[4]) + ">"
        elif nozzle_n == 2:
            initialization = "<PUMP3, " + speed + ", " + str(self.motor_values[0]) + ">"
            remove_drip = "<PUMP3, " + speed + ", " + str(self.motor_values[1]) + ">"
            dispense = "<PUMP3, " + speed + ", " + str(self.motor_values[2]) + ">"
            remove_drip2 = "<PUMP3, " + speed + ", " + str(self.motor_values[3]) + ">"
            idle = "<PUMP3, " + speed + ", " + str(self.motor_values[4]) + ">"
...
class Rinse(Motor):
    def __init__(self, loop=1, wait_time=[16, 12, 15, 12, 16]):
        # global state
        self.wait_time = wait_time
        self.loop = loop
        Motor.__init__(self)
    ...
    def run(self):
        # global state
        move_down = b"<ZFLUSH, 100, +72500>"
        suc = b"<DC2, 150, 9000>"
        flush = b"<PUMP4,1000,9800>"
        move_up = b"<ZFLUSH, 100, -72500>"
...
class Rinse_dummy(Motor):  # rinse function for the dummy cell
    def __init__(self, loop=1, wait_time=[16, 12, 15, 12, 16]):
        # global state
        self.wait_time = wait_time
        self.loop = loop
    ...
    def run(self):
        # global state
        move_down = b"<ZFLUSH, 100, +68500>"
        suc = b"<DC2, 150, 9000>"
        flush = b"<PUMP4,1000,9800>"
        move_up = b"<ZFLUSH, 100, -68500>"
...
class Dry(Motor):
    def __init__(self, wait_time=[7, 20, 7]):
        # global state
        self.wait_time = wait_time
        Motor.__init__(self)
        self.state = 3
...
class N2(Motor):
    def __init__(
        self, nozzle=1, loop=1, wait_time=[12, 3, 12], mode="single"
    ):  # [221206] from [0,0,0]
        # global state
        self.nozzle = nozzle
        self.wait_time = wait_time
    ...
    def run(self):
        ...
        if self.nozzle == 2:
            ...
            if self.wait_time[0]:
                self.send(b"<ZAIRDRY, 100, +60000>")
                time.sleep(self.wait_time[0])
            if self.wait_time[1]:
                if self.mode == "dual":
                    for x in range(self.loop):
                        self.send(b"<DC6, 255, 110000>")
                        time.sleep(5)
                        self.send(b"<DC3, 255, 100000>")  # ~70 seconds
                        time.sleep(75)
                    time.sleep(self.wait_time[1])
            ...
            if self.wait_time[1]:
                if self.mode == "dual":
                    for x in range(self.loop):
                        ...
                        time.sleep(5)
                        self.send(b"<DC3, 255, 8000>")  # ~5 seconds
                        time.sleep(12)
                    time.sleep(self.wait_time[1])
                else:  # By default, mode = 'single'
                    for x in range(self.loop):
